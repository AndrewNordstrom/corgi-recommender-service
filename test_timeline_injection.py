...
def test_uniform_injection():
    ...
    print(f"Starting with {len(real_posts)} real posts and {len(injectable_posts)} injectable posts")
    
    # Define uniform strategy
    strategy = {
        "type": "uniform",
        "max_injections": 2,
        "shuffle_injected": False
    }
    ...
    print(f"After injection: {len(merged_timeline)} total posts, {injected_count} injected posts")
    ...
    for i, post in enumerate(merged_timeline):
        ...
        print(f"{i+1}. [{post_type}] ID: {post['id']}, Created At: {post['created_at']}")
