...
def print_result(check_name, success, message=None, error=None):
    ...
    if message and success:
        ...
        redacted_message = re.sub(
            r"(password|token|key|secret)=\S+", r"\1=*****", str(message)
        )
    ...
    if error and not success:
        ...
        redacted_error = re.sub(
            r"(password|token|key|secret)=\S+", r"\1=*****", str(error)
        )
    ...
    try:
        ...
        if response.status_code == 200:
            try:
                ...
                return print_result(
                    "API Health", True, f"API is running (latency: {latency:.2f}ms)"
                )
        else:
            return print_result(
                "API Health",
...
def check_db_connection():
    ...
    try:
        ...
        return print_result(
            "Database Connection", False, error=f"Database check failed: {str(e)}"
        )
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            if not missing_tables:
                ...
                counts_str = ", ".join(
                    [f"{table}: {count} rows" for table, count in table_counts.items()]
                )
                conn.close()
                return print_result(
                    "Database Schema", True, f"All required tables exist. {counts_str}"
                )
            else:
                conn.close()
                missing_str = ", ".join(missing_tables)
                return print_result(
                    "Database Schema", False, error=f"Missing tables: {missing_str}"
                )
    except Exception as e:
        return print_result(
            "Database Schema", False, error=f"Schema check failed: {str(e)}"
        )
...
def check_environment():
    ...
    if not missing_vars:
        ...
        return print_result(
            "Environment", False, error=f"Missing environment variables: {missing_str}"
        )
...
def main():
    ...
    if checks_passed == checks_total:
        print(
            f"{GREEN}{BOLD}✅ All checks passed ({checks_passed}/{checks_total}){RESET}"
        )
        exit_code = 0
    else:
        print(
            f"{RED}{BOLD}❌ {checks_total - checks_passed} check(s) failed ({checks_passed}/{checks_total} passed){RESET}"
        )
        exit_code = 1
