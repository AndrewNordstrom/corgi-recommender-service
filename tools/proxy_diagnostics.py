...
def parse_args():
    ...
    parser.add_argument('--user-id', 
                        help='User ID to use for the request')
    parser.add_argument('--instance', default='mastodon.social',
                        help='Mastodon instance to target (default: mastodon.social)')
    parser.add_argument('--path', default='timelines/home',
                        help='API path to request (default: timelines/home)')
    parser.add_argument('--token', 
                        help='OAuth token to use for authentication')
...
def analyze_result(result):
    ...
    if result['success'] and result['response']:
        ...
        if isinstance(result['response'], list):
            actual_recs = sum(1 for post in result['response'] if post.get('is_recommendation', False))
    ...
    auth_headers_present = any(h.lower() == 'authorization' for h in result.get('headers', {}).keys())
...
def format_output(analysis, result, args):
    ...
    if analysis['success']:
        ...
        lines.append(f"Recommendations Injected: {analysis['recommendations_injected']}")
