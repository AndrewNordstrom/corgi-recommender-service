...
class CorgiValidator:
    ...
    def seed_posts(self, count: int = DEFAULT_NUM_POSTS) -> List[Dict]:
        ...
        for i in range(count):
            ...
            interaction_counts = {"favorites": 0, "reblogs": 0, "replies": 0}
            ...
            if status_code in (200, 201):
                ...
                logger.error(
                    f"Failed to create post: {status_code}, {json.dumps(response)}"
                )
                ...
                try:
                    ...
                    status_code, response = self._make_request(
                        "POST", "/v1/posts", fallback_payload
                    )

                    if status_code not in (200, 201):
                        # Fallback 2: Try with 'text' instead of 'content'
                        logger.warning("Trying second fallback post schema...")
                        fallback_payload = {"text": post["content"], "language": "en"}
                        logger.debug(
                            f"Fallback payload 2: {json.dumps(fallback_payload)}"
                        )
                        status_code, response = self._make_request(
                            "POST", "/v1/posts", fallback_payload
                        )
                    ...
                    if status_code not in (200, 201):
                        ...
                        fallback_payload = {"status": post["content"]}
                        logger.debug(
                            f"Fallback payload 3: {json.dumps(fallback_payload)}"
                        )
                        status_code, response = self._make_request(
                            "POST", "/v1/posts", fallback_payload
                        )
                    ...
                    if status_code in (200, 201):
                        logger.info(
                            f"Created synthetic post {i+1}/{count} using fallback: {post_id}"
                        )
                        posts.append(post)
    ...
    def simulate_interactions(
        self, user_count: int = None, post_count: int = None
    ) -> List[Dict]:
        """
        ...
        users = (
            self.synthetic_users[:user_count] if user_count else self.synthetic_users
        )
        posts = (
            self.synthetic_posts[:post_count] if post_count else self.synthetic_posts
        )

        logger.info(
            f"Simulating interactions for {len(users)} users with {len(posts)} posts..."
        )
        ...
        for user_id in users:
            ...
            for post in selected_posts:
                ...
                action_type = random.choices(action_types, weights=action_weights, k=1)[
                    0
                ]
                ...
                if status_code in (200, 201):
                    logger.info(
                        f"Created interaction: {user_id} {action_type} {post['post_id']}"
                    )
                    interactions.append(interaction)
                else:
                    logger.error(
                        f"Failed to create interaction: {status_code}, {response}"
                    )
    ...
    def validate_recommendation_format(self, recs: List[Dict]) -> List[str]:
        ...
        for i, post in enumerate(recs):
            ...
            if not post.get("id") or not isinstance(post.get("id"), str):
                errors.append(
                    f"Post {post_id} missing or invalid id (should be non-empty string)"
                )

            if not post.get("content") or not isinstance(post.get("content"), str):
                errors.append(
                    f"Post {post_id} missing or invalid content (should be non-empty string or HTML)"
                )
            ...
            if not language or not isinstance(language, str) or len(language) < 2:
                errors.append(
                    f"Post {post_id} missing or invalid language (should be ISO language string like 'en')"
                )
            ...
            if not created_at or not isinstance(created_at, str):
                errors.append(
                    f"Post {post_id} missing or invalid created_at (should be ISO timestamp string)"
                )
            else:
                try:
                    ...
                    errors.append(
                        f"Post {post_id} has invalid created_at timestamp format: {created_at}"
                    )
            ...
            if ranking_score is None:
                ...
                not isinstance(ranking_score, (int, float))
                or ranking_score < 0.0
                or ranking_score > 1.0
            ):
                errors.append(
                    f"Post {post_id} has invalid ranking_score: {ranking_score} (should be float between 0.0-1.0)"
                )
            ...
            if not reason or not isinstance(reason, str):
                errors.append(
                    f"Post {post_id} missing or invalid recommendation_reason (should be non-empty string)"
                )
            ...
            for flag in ["is_real_mastodon_post", "is_synthetic"]:
                if flag not in post or not isinstance(post[flag], bool):
                    errors.append(
                        f"Post {post_id} missing or invalid {flag} (should be boolean)"
                    )
            ...
            if not account or not isinstance(account, dict):
                ...
                if not account.get("username") or not isinstance(
                    account.get("username"), str
                ):
                    errors.append(
                        f"Post {post_id} has invalid account.username (should be non-empty string)"
                    )

                # Account display_name validation
                if "display_name" not in account or not isinstance(
                    account.get("display_name"), str
                ):
                    errors.append(
                        f"Post {post_id} has invalid account.display_name (should be string)"
                    )
                ...
                if not url or not isinstance(url, str):
                    errors.append(
                        f"Post {post_id} missing or invalid account.url (should be valid URL)"
                    )
                else:
                    # Simple URL validation
                    if not (url.startswith("http://") or url.startswith("https://")):
                        errors.append(
                            f"Post {post_id} has invalid account.url format: {url}"
                        )
    ...
    def check_recommendations(self, user_id: Optional[str] = None) -> Dict:
        ...
        status_code, response = self._make_request(
            "POST", "/v1/recommendations/rankings/generate", {"user_id": check_user}
        )
        ...
        status_code, response = self._make_request(
            "GET", "/v1/recommendations", params={"user_id": check_user}
        )
        ...
        if user_interactions:
            positive_post_ids = [
                ...
                if i["action_type"]
                in ["favorite", "bookmark", "reblog", "more_like_this"]
            ]
            ...
            negative_in_recs = [
                post_id for post_id in negative_post_ids if post_id in recommended_ids
            ]

            if negative_in_recs:
                logger.warning(
                    f"Found {len(negative_in_recs)} posts in recommendations that user indicated 'less_like_this'"
                )
                return {
                    "status": "warn",
        ...
        return {
            ...
            "recommendations_count": len(recommendations),
            "sample_reasons": [
                rec["recommendation_reason"] for rec in recommendations[:3]
            ],
        }
    ...
    def check_feedback_logging(self, feedback_count: int = 3) -> Dict:
        ...
        for _ in range(feedback_count):
            ...
            feedback_type = random.choice(
                ["favorite", "more_like_this", "less_like_this"]
            )
        ...
        for entry in feedback_entries:
            ...
            if status_code != 200:
                logger.error(
                    f"Failed to retrieve interactions: {status_code}, {response}"
                )
                all_verified = False
            ...
            for interaction in response.get("interactions", []):
                if (
                    interaction["post_id"] == entry["post_id"]
                    and interaction["action_type"] == entry["action_type"]
                ):
                    found = True
                    break
            ...
            if not found:
                ...
                all_verified = False
                failed_entries.append(
                    {"entry": entry, "error": "Entry not found in user interactions"}
                )
    ...
    def check_privacy_modes(self) -> Dict:
        ...
        if not any(i["user_id"] == test_user for i in self.interactions):
            ...
            for i in range(3):
                ...
                if status_code not in (200, 201):
                    logger.error(
                        f"Failed to create interaction for privacy test: {status_code}, {response}"
                    )
                    return {
                        "status": "fail",
                        "error": "Could not create test interactions",
                    }
        ...
        for level in privacy_levels:
            ...
            status_code, response = self._make_request(
                "POST",
                "/v1/privacy/settings",
                {"user_id": test_user, "tracking_level": level},
            )

            if status_code != 200:
                logger.error(
                    f"Failed to set privacy level to {level}: {status_code}, {response}"
                )
                return {
                    "status": "fail",
            ...
            status_code, response = self._make_request(
                "GET", "/v1/privacy/settings", params={"user_id": test_user}
            )

            if status_code != 200:
                logger.error(
                    f"Failed to retrieve privacy settings: {status_code}, {response}"
                )
                return {
                    "status": "fail",
            ...
            if response.get("tracking_level") != level:
                logger.error(
                    f"Privacy level mismatch: expected {level}, got {response.get('tracking_level')}"
                )
                return {
                    "status": "fail",
            ...
            status_code, response = self._make_request(
                "GET", f"/v1/interactions/user/{test_user}"
            )
            ...
            if status_code == 404 and level == "none":
                ...
                logger.error(
                    f"Failed to retrieve interactions with privacy level {level}: {status_code}, {response}"
                )
                return {
                    "status": "fail",
            ...
            if level == "full":
                ...
                if "interactions" not in response or not isinstance(
                    response["interactions"], list
                ):
                    logger.error(
                        f"Expected detailed interactions with privacy level {level}, but got: {response}"
                    )
                    return {
                        "status": "fail",
                ...
                if "interaction_counts" not in response:
                    logger.error(
                        f"Expected aggregated counts with privacy level {level}, but got: {response}"
                    )
                    return {
                        "status": "fail",
                ...
                if (
                    "privacy_level" not in response
                    or response.get("privacy_level") != "none"
                ):
                    logger.error(
                        f"Expected privacy_level=none indication in response, but got: {response}"
                    )
                    return {
                        "status": "fail",
            ...
            privacy_checks[level] = {
                ...
                "has_detailed_data": "interactions" in response
                and len(response.get("interactions", [])) > 0,
        ...
        self._make_request(
            "POST",
            "/v1/privacy/settings",
            {"user_id": test_user, "tracking_level": "full"},
        )
    ...
    def check_blending(self) -> Dict:
        ...
        status_code, _ = self._make_request(
            "POST", "/v1/recommendations/rankings/generate", {"user_id": test_user}
        )

        # Get regular timeline
        status_code, timeline_response = self._make_request(
            "GET", "/v1/timelines/home", params={"user_id": test_user}
        )

        if status_code != 200:
            # The endpoint might not exist yet, which is fine
            logger.info(
                f"Timeline endpoint returned {status_code} - this may be normal if not implemented yet"
            )
        ...
        regular_timeline_count = (
            len(timeline_response.get("timeline", []))
        ...
        rec_markers = [
            post for post in augmented_posts if post.get("is_recommendation") is True
        ]
    ...
    def run_all_checks(
        self, users_count: int = DEFAULT_NUM_USERS, posts_count: int = DEFAULT_NUM_POSTS
    ) -> Dict:
        """
        ...
        summary = {
            ...
            "checks": {check: result["status"] for check, result in checks.items()},
        }
    ...
    def print_results(self) -> None:
        ...
        for check, result in self.check_results.items():
            ...
            if (
                ...
                and status == "fail"
                and "validation_errors" in result
            ):
                print("\nRecommendation format validation errors:")
                for i, err in enumerate(
                    result["validation_errors"][:10]
                ):  # Limit to first 10 errors
                    print(f"  {i+1}. {err}")

                if len(result["validation_errors"]) > 10:
                    print(
                        f"  ... and {len(result['validation_errors']) - 10} more errors (see log for details)"
                    )
                print()
...
def main():
    ...
    # Basic options
    parser.add_argument(
        "--api-base", default=DEFAULT_API_BASE, help="Base URL for the Corgi API"
    )
    parser.add_argument(
        "--api-prefix", default="/api/v1", help="API endpoint prefix (default: /api/v1)"
    )
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging")
    parser.add_argument(
        "--dry-run", action="store_true", help="Simulate without making actual requests"
    )
    parser.add_argument(
        "--skip-server-check", action="store_true", help="Skip the server status check"
    )

    # Seed options
    parser.add_argument(
        "--run-seed", type=int, help="Number of fake users/posts to generate"
    )
    ...
    parser.add_argument(
        "--check-recs",
        action="store_true",
        help="Check if recommendations are returned",
    )
    parser.add_argument(
        "--check-feedback", action="store_true", help="Check if feedback routes work"
    )
    parser.add_argument(
        "--check-privacy",
        action="store_true",
        help="Check if privacy levels function correctly",
    )
    parser.add_argument(
        "--check-blend",
        action="store_true",
        help="Check if timeline + rec blending works",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--check-server",
        action="store_true",
        help="Only check if the server is running",
    )
    parser.add_argument(
        "--check-paths",
        action="store_true",
        help="Check available API paths to find the correct configuration",
    )
    ...
    if not args.skip_server_check and not args.dry_run:
        try:
            ...
            if response.status_code != 200:
                print(
                    f"⚠️  WARNING: Server health check failed with status {response.status_code}"
                )
                ...
                if not server_running:
                    ...
                    print(
                        "Run with --dry-run to simulate without a server or start the server first."
                    )
                    return
    ...
    if args.run_seed and not run_any_check:
        ...
        print(
            f"Seeded {args.run_seed} users and {args.run_seed * 2} posts with interactions"
        )
        return
    ...
    if args.check_all or not run_any_check:
        ...
        if args.check_recs:
            validator.check_results["recommendations"] = (
                validator.check_recommendations()
            )
...
def check_api_paths():
    ...
    if working_endpoints:
        ...
        print(
            "Try starting it with: HOST=0.0.0.0 PORT=5001 python3 -m flask --app app run"
        )
...
def check_server_status():
    ...
    try:
        port_5000_status = subprocess.run(
            ["lsof", "-i", ":5000"], capture_output=True, text=True
        )
        if port_5000_status.stdout:
            print("\n⚠️  WARNING: Port 5000 is already in use:")
            print(port_5000_status.stdout)
            print(
                "This is likely Apple Control Center/AirTunes which conflicts with the default Flask port."
            )
    ...
    try:
        flask_processes = subprocess.run(
            ["ps", "aux"], capture_output=True, text=True
        ).stdout
        if "flask" in flask_processes.lower() or "gunicorn" in flask_processes.lower():
            print(
                "A Flask or Gunicorn process seems to be running, but not accessible at the expected URL."
            )
        else:
...
def test_manual_post():
    ...
    payloads = [
        ...
        {"text": "Test post from validator harness - text field", "language": "en"},
    ]
        ...
        try:
            if "query" in auth:
                response = requests.post(
                    ...
                    params=auth.get("query", {}),
                )
            else:
                response = requests.post(
                    url, json=payloads[0], headers=auth.get("headers", {})
                )
    ...
    if any(auth for auth in auth_methods if "SUCCESS" in auth):
        ...
        for i, payload in enumerate(payloads):
            ...
            try:
                ...
                successful_auth = next(
                    (auth for auth in auth_methods if "SUCCESS" in auth),
                    auth_methods[0],
                )
                ...
                if "query" in successful_auth:
                    response = requests.post(
                        url,
                        json=payload,
                        headers=successful_auth.get("headers", {}),
                        params=successful_auth.get("query", {}),
                    )
                else:
                    response = requests.post(
                        url, json=payload, headers=successful_auth.get("headers", {})
                    )
