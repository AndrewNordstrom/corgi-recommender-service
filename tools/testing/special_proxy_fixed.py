...
COLD_START_POSTS_PATH = os.path.join(
    os.path.dirname(__file__), "data", "cold_start_posts.json"
)
...
def handle_exception(e):
    ...
    return (
        jsonify({"status": "error", "message": "An internal server error occurred"}),
        500,
    )
...
def add_cors_headers(response):
    ...
    if origin:
        ...
        )  # 24 hours for preflight

    # Log the request info for debugging
    logger.debug(
        f"Request: {request.method} {request.path} | Origin: {origin} | Response: {response.status_code}"
    )
    ...
    return jsonify(
        {
            ...
            "version": "4.1.1",
            "urls": {"streaming_api": "wss://localhost:5004"},
            "stats": {"user_count": 10, "status_count": 100, "domain_count": 1},
...
def api_tag_timeline(tag_name):
    ...
    for i, post in enumerate(cold_start_posts[:5]):
        ...
        if not any(t.get("name") == tag_name for t in post_copy["tags"]):
            post_copy["tags"].append(
                {"name": tag_name, "url": f"https://mastodon.social/tags/{tag_name}"}
            )
        ...
        if "account" in post_copy and isinstance(post_copy["account"], dict):
            ...
            if "acct" not in account:
                if "@" in account.get("username", ""):
                    ...
                    account["acct"] = (
                        f"{account.get('username', 'user')}@mastodon.social"
                    )
    ...
    logger.info(
        f"TAG-{request_id} | Generated {len(tag_posts)} posts for tag #{tag_name}"
    )
...
def create_complete_status(status_id, account_id="user123"):
    ...
    return {
        ...
        "tags": [],
        "emojis": [],
        "application": {"name": "Corgi Recommender"},
        "account": {
            "id": account_id,
            "username": "andrewn",
...
def api_get_status(status_id):
    ...
    """
    logger.info(
        f"Status request received for ID {status_id} from: {request.remote_addr}"
    )
...
def api_status_context(status_id):
    ...
    """
    logger.info(
        f"Status context request for ID {status_id} from: {request.remote_addr}"
    )
...
def api_status_reblogs(status_id):
    ...
    """
    logger.info(
        f"Status reblogs request for ID {status_id} from: {request.remote_addr}"
    )
...
@app.route(
    "/api/v1/<path:path>", methods=["GET", "POST", "PUT", "DELETE", "OPTIONS", "PATCH"]
)
...
def proxy_to_mastodon(path):
    ...
    try:
        ...
        if "application/json" in proxied_response.headers.get("Content-Type", ""):
            try:
                ...
                if path == "timelines/home" and isinstance(response_json, list):
                    logger.info(
                        f"ENHANCING-{request_id} | Adding recommendations to home timeline with {len(response_json)} posts"
                    )
                    ...
                    if cold_start_posts:
                        ...
                        for i, post in enumerate(cold_start_posts[:2]):
                            ...
                            post_copy["recommendation_reason"] = (
                                f"Recommended by Corgi for {user_id}"
                            )
                            post_copy["filtered"] = []
                            ...
                            if "account" in post_copy and isinstance(
                                post_copy["account"], dict
                            ):
                                account = post_copy["account"]
                                # Required fields for Elk
                                if "acct" not in account and "username" in account:
                                    account["acct"] = (
                                        f"{account['username']}@mastodon.social"
                                    )

                                if "url" not in account and "username" in account:
                                    account["url"] = (
                                        f"https://mastodon.social/@{account['username']}"
                                    )
                    ...
                    logger.info(
                        f"ENHANCED-{request_id} | Returning enhanced timeline with {len(response_json)} posts"
                    )
        ...
        response = Response(
            proxied_response.content, status=status_code, headers=response_headers
        )
        ...
        if path == "timelines/home":
            logger.info(
                f"FALLBACK-{request_id} | Using cold start posts as fallback for timeline"
            )
            cold_start_posts = load_cold_start_posts()
            posts_to_return = []
            ...
            logger.info(
                f"FALLBACK-{request_id} | Returning {len(posts_to_return)} cold start posts as fallback"
            )
        ...
        if "notifications" in path:
            ...
            return jsonify(
                {"error": "Could not connect to Mastodon instance", "details": str(e)}
            )
...
def ensure_mastodon_post_format(post, user_id=None):
    ...
    logger.debug(
        f"Validated post {post.get('id', 'unknown')}: username={account.get('username', 'missing')}, "
...
def get_home_timeline():
    ...
    try:
        ...
        if cold_start:
            logger.info(
                f"TIMELINE-{request_id} | Using cold start mode for user {user_id}"
            )

            # Load the actual cold start posts
            cold_start_posts = load_cold_start_posts()
            logger.info(
                f"TIMELINE-{request_id} | Loaded {len(cold_start_posts)} posts from cold start database"
            )
            ...
            for post in cold_start_posts[:20]:  # Limit to 20 posts
                ...
                if "id" not in post:
                    post["id"] = (
                        f"cold-{len(posts_to_return)}-{hash(str(post)) % 10000}"
                    )
            ...
            logger.info(
                f"TIMELINE-{request_id} | Returning {len(posts_to_return)} actual cold start posts"
            )
            ...
            for i, post in enumerate(posts_to_return[:2]):
                if "account" in post and isinstance(post["account"], dict):
                    account = post["account"]
                    logger.info(
                        f"TIMELINE-{request_id} | Post {i}: id={post.get('id')}, "
        ...
        proxied_response = requests.get(
            url=target_url, headers=headers, params=params, timeout=10
        )
            ...
            try:
                ...
                if len(timeline_data) < 3:
                    ...
                    for post in cold_start_posts[:5]:
                        ...
                        if "id" not in post:
                            post["id"] = (
                                f"rec-{recommended_count}-{hash(str(post)) % 10000}"
                            )
                ...
                for i, post in enumerate(timeline_data):
                    ...
                    if "account" in post and isinstance(post["account"], dict):
                        ...
                        if (
                            i < 3
                        ):  # Only log details for the first 3 posts to avoid spamming logs
                            logger.info(
                                f"TIMELINE-{request_id} | Post {i+1}: id={post.get('id', 'unknown')}, "
                ...
                logger.info(
                    f"TIMELINE-{request_id} | Successfully retrieved and enhanced timeline with {len(timeline_data)} posts"
                )
        ...
        for i in range(1, 6):
            ...
            post["recommendation_reason"] = (
                "Suggested while we couldn't reach your home timeline"
            )
            fallback_posts.append(post)

        # Log the fallback
        logger.info(
            f"TIMELINE-{request_id} | Returning {len(fallback_posts)} mock posts as fallback"
        )
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Run the Special Corgi Proxy Server for Elk integration"
    )
    parser.add_argument(
        "--host", type=str, help="Host to bind to (overrides environment variables)"
    )
    parser.add_argument(
        "--port", type=int, help="Port to bind to (overrides environment variables)"
    )
    parser.add_argument("--no-https", action="store_true", help="Disable HTTPS")
    parser.add_argument(
        "--force-http",
        action="store_true",
        help="Force HTTP even if HTTPS is configured but certs are missing",
    )
    parser.add_argument(
        "--cert",
        type=str,
        help="Path to SSL certificate (overrides environment variables)",
    )
    parser.add_argument(
        "--key", type=str, help="Path to SSL key (overrides environment variables)"
    )
...
def nodeinfo():
    ...
    return jsonify(
        {
            "version": "2.0",
            "software": {"name": "corgi-recommender", "version": "1.0.0"},
...
def add_static_routes():
    ...
    def serve_test_client():
        ...
        with open(
            os.path.join(os.path.dirname(__file__), "test_client.html"), "r"
        ) as f:
            return f.read()
    ...
    def serve_elk_integration_js():
        ...
        with open(
            os.path.join(os.path.dirname(__file__), "elk_integration.js"), "r"
        ) as f:
            content = f.read()
    ...
    def serve_elk_integration_guide():
        ...
        with open(
            os.path.join(os.path.dirname(__file__), "elk_integration_guide.html"), "r"
        ) as f:
            content = f.read()
        return content
...
def validate_ssl_config(cert_path, key_path, force_http=False):
    ...
    if os.path.exists(cert_path) and os.path.exists(key_path):
        ...
        try:
            ...
            logger.error(
                "Missing required packages for HTTPS. Install with: pip install pyopenssl cryptography"
            )
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info(
            "Could not import from config.py, using environment variables and defaults"
        )
    ...
    if not os.path.exists(DB_FILE):
        logger.error(
            f"Database file {DB_FILE} does not exist. Run simplified_setup.py first."
        )
        sys.exit(1)
    ...
    if not os.path.exists(COLD_START_POSTS_PATH):
        logger.warning(
            f"Cold start posts file {COLD_START_POSTS_PATH} does not exist. Cold start will not work."
        )
    else:
    ...
    if use_https:
        use_https, ssl_context = validate_ssl_config(
            cert_path, key_path, args.force_http
        )
    else:
        ssl_context = None
    ...
    logger.info(
        f"- {server_url}/test-corgi-response (Debug endpoint for Elk rendering)"
    )
    logger.info("")
    ...
    logger.info(
        f"Profile display integration guide: {server_url}/elk-integration-guide"
    )
