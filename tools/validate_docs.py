...
def validate_mkdocs_config():
    ...
    try:
        ...
        if missing_keys:
            print(
                colored(
                    f"❌ mkdocs.yml is missing required keys: {', '.join(missing_keys)}",
                    "red",
                )
            )
            return False
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Validate Corgi Recommender Service documentation"
    )
    parser.add_argument(
        "--check-only",
        action="store_true",
        help="Only check files, don't suggest actions",
    )
    args = parser.parse_args()

    print(
        colored(
            "\n=== Corgi Recommender Documentation Validation ===",
            "cyan",
            attrs=["bold"],
        )
    )
    ...
    if overall_status:
        ...
        if not args.check_only:
            ...
            if check_mkdocs_command():
                print(
                    colored(
                        "\nSuggestion: Run the following to serve the documentation locally:",
                        "cyan",
                    )
                )
                print(
                    colored(
                        "    cd " + str(PROJECT_ROOT) + " && mkdocs serve", "yellow"
                    )
                )
            else:
                print(
                    colored(
                        "\nSuggestion: Install mkdocs to serve the documentation locally:",
                        "cyan",
                    )
                )
        ...
        print(
            colored(
                "❌ Some documentation files or configurations are missing or invalid",
                "red",
            )
        )
        ...
        if not mkdocs_valid and not args.check_only:
            print(
                colored(
                    "\nFix the mkdocs.yml configuration file to enable documentation serving",
                    "cyan",
                )
            )
