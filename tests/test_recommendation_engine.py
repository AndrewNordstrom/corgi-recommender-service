...
def test_get_ranked_recommendations(
    mock_load_cold_start,
    mock_is_new_user,
    mock_get_db_connection,
    mock_generate_rankings,
    mock_ranking_data,
    mock_cold_start_data,
):
    ...
    assert (
        posts[0]["injection_metadata"]["score"]
        > posts[1]["injection_metadata"]["score"]
    )
    assert (
        posts[1]["injection_metadata"]["score"]
        > posts[2]["injection_metadata"]["score"]
    )
...
@patch("utils.recommendation_engine.get_db_connection")
@patch("utils.recommendation_engine.is_new_user")
def test_recommendations_with_mastodon_data(
    mock_is_new_user, mock_get_db_connection, mock_generate_rankings, mock_ranking_data
):
