...
def test_get_content_engagement_score():
    ...
    high_engagement_post = {
        "interaction_counts": {"favorites": 100, "reblogs": 50, "replies": 25}
    }
    high_score = get_content_engagement_score(high_engagement_post)

    # Test with low engagement
    low_engagement_post = {
        "interaction_counts": {"favorites": 1, "reblogs": 0, "replies": 0}
    }
...
def test_calculate_ranking_score():
    ...
    with patch(
        "core.ranking_algorithm.get_author_preference_score"
    ) as mock_author_score:
        with patch(
            "core.ranking_algorithm.get_content_engagement_score"
        ) as mock_engagement_score:
            with patch(
                "core.ranking_algorithm.get_recency_score"
            ) as mock_recency_score:
...
def test_generate_rankings_for_user(mock_generate_alias, mock_get_conn, mock_db_conn):
    ...
    with patch("core.ranking_algorithm.get_author_preference_score", return_value=0.5):
        with patch(
            "core.ranking_algorithm.get_content_engagement_score", return_value=0.6
        ):
            with patch("core.ranking_algorithm.get_recency_score", return_value=0.7):
                ...
                mock_cursor.execute.assert_any_call(
                    ...
                    (
                        ...
                        result[0]["recommendation_reason"],
                    ),
                )
