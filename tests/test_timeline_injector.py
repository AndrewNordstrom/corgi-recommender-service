...
def test_get_post_timestamp():
    ...
    assert (
        abs((extracted - now).total_seconds()) < 1
...
def test_sort_posts_by_timestamp(test_posts):
    ...
    for i in range(len(sorted_posts) - 1):
        assert get_post_timestamp(sorted_posts[i]) >= get_post_timestamp(
            sorted_posts[i + 1]
        )
...
def test_extract_tags():
    ...
    post = {"tags": [{"name": "tech"}, {"name": "programming"}]}
...
def test_uniform_strategy(test_posts):
    ...
    result = inject_into_timeline(
        test_posts["real"], test_posts["injectable"], strategy
    )
    ...
    for post in result:
        if post.get("injected", False):
            assert (
                "injection_metadata" in post
                or post.get("is_real_mastodon_post") is False
            )


def test_after_n_strategy(test_posts):
    """Test injecting after every N posts."""
    strategy = {"type": "after_n", "n": 2, "max_injections": 4}
...
def test_first_only_strategy(test_posts):
    ...
    strategy = {"type": "first_only", "max_injections": 2}
    ...
    injected_positions = [
        i for i, post in enumerate(result) if post.get("injected", False)
    ]
    ...
    for pos in injected_positions:
        assert (
            pos < len(result) // 2
        ), f"Injected post at position {pos} should be in first half of timeline"
...
def test_tag_match_strategy(test_posts):
    ...
    strategy = {"type": "tag_match", "max_injections": 3}
...
def test_shuffle_injected(test_posts):
    ...
    random.seed(42)
    result1 = inject_into_timeline(
        test_posts["real"], test_posts["injectable"], strategy
    )

    random.seed(42)
    result2 = inject_into_timeline(
        test_posts["real"], test_posts["injectable"], strategy
    )

    # Results should be identical with the same seed
    assert [p["id"] for p in result1 if p.get("injected")] == [
        p["id"] for p in result2 if p.get("injected")
    ]

    # Change seed and results should differ
    random.seed(100)
    result3 = inject_into_timeline(
        test_posts["real"], test_posts["injectable"], strategy
    )
...
def test_empty_inputs():
    ...
    result = inject_into_timeline(
        [{"id": "1", "created_at": "2023-01-01T00:00:00Z"}], [], {"type": "uniform"}
    )
    assert len(result) == 1
...
def test_recommendation_engine(
    mock_generate_rankings, mock_get_db_connection, test_posts
):
    ...
    strategy = {"type": "uniform", "max_injections": 5}
