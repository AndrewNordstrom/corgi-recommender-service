...
def main():
    ...
    parser.add_argument('--iterations', default=10, type=int, help='Number of test iterations')
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if timeline and len(timeline) > 0:
                    ...
                    action_type = random.choice(['favorite', 'reblog', 'bookmark', 'more_like_this'])
                    ...
                    print(f"  👆 Logging {action_type} interaction with post {post_id} (injected: {is_injected})")
                    interaction_response = requests.post(interaction_url, json=interaction_data)
                    
                    if interaction_response.status_code == 200:
                        print(f"  ✅ Interaction logged successfully")
                    else:
                        print(f"  ❌ Failed to log interaction: {interaction_response.status_code}")
            else:
