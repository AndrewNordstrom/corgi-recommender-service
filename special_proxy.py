...
if __name__ == "__main__":
    ...
    try:
        ...
        # Use environment-aware host binding for security
        env = os.getenv("FLASK_ENV", "development").lower()
        default_host = "localhost" if env == "production" else "0.0.0.0"  # nosec B104 - Intentionally safe for development
        
        HOST = os.getenv("CORGI_HOST", os.getenv("HOST", default_host))
    ...
    server_url = f"{protocol}://{host if host != '0.0.0.0' else 'localhost'}:{port}"  # nosec B104 - URL display logic, not binding
