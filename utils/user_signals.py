...
PROMOTION_THRESHOLD_UNIQUE_TAGS = 3   # Default number of unique tags with >=2 interactions
...
_CONFIG_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), 
                          'config', 'cold_start_weights.json')
...
def update_user_signals(user_id: str, post_metadata: Dict[str, Any], 
                        action_type: str) -> Dict[str, Counter]:
    """
    ...
    with _signal_lock:
        ...
        for signal_type in SIGNAL_TYPES:
            ...
            if signal_type == "tags" and "tags" in post_metadata:
                ...
                if isinstance(tags, list):
                    for tag in tags:
                        ...
                        signal_updates[signal_type][tag] = _user_signals[user_alias][signal_type][tag]
            ...
            # Handle category separately (common field)
                ...
                if signal_type not in signal_updates:
                    signal_updates[signal_type] = {}
                signal_updates[signal_type][category] = _user_signals[user_alias][signal_type][category]
            ...
            # Handle scalar fields (vibe, tone, etc.)
                ...
                if signal_type not in signal_updates:
                    signal_updates[signal_type] = {}
                signal_updates[signal_type][value] = _user_signals[user_alias][signal_type][value]
...
def check_promotion_status(user_alias: str) -> bool:
    ...
    with _signal_lock:
        ...
        should_promote = (
            total_interactions >= PROMOTION_THRESHOLD_INTERACTIONS and
            tags_with_multiple >= PROMOTION_THRESHOLD_UNIQUE_TAGS
        )
...
def get_weighted_post_selection(user_id: str, posts: List[Dict[str, Any]], 
    ...
    if weighted_count > 0 and user_alias in _signal_history and len(_signal_history[user_alias]) > 0:
        ...
        additional_random = random.sample(
            remaining_posts, 
            min(weighted_count, len(remaining_posts))
        )
...
def calculate_post_score(post: Dict[str, Any], user_signals: Dict[str, Counter]) -> float:
    """
    ...
    for signal_type, weight in type_weights.items():
        ...
        if (signal_type not in user_signals or 
            not user_signals[signal_type] or
            signal_type not in post and signal_type != "category"):
            continue
...
def log_post_selection(user_alias: str, selected_posts: List[Dict[str, Any]], 
                      random_ratio: float, weighted_ratio: float) -> None:
    """
            ...
            with get_db_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    for user_alias, signals in _user_signals.items():
                        ...
                        json_data = {
                            k: dict(v) for k, v in signals.items() if v
                        }
                    ...
                    logger.info(f"Exported {len(_user_signals)} user signal profiles to database")
                    return True
        ...
        with _signal_lock:
            with get_db_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    logger.info(f"Imported {len(rows)} user signal profiles from database")
                    return True
