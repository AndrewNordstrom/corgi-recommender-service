...
def generate_user_alias(user_id):
    ...
    if not USER_HASH_SALT:
        if os.getenv('FLASK_ENV') == 'production':
            ...
            logger.error("CRITICAL SECURITY ERROR: Empty salt in production! Set USER_HASH_SALT environment variable.")
...
def get_user_privacy_level(conn, user_id):
    ...
    with get_cursor(conn) as cur:
        ...
        query = "SELECT tracking_level FROM privacy_settings WHERE user_id = " + placeholder
