...
def check_time_gap(before_post: Dict, after_post: Dict, min_gap_minutes: int) -> bool:
    """
    ...
    if meets_requirement:
        logger.debug(
            f"Gap of {gap_minutes:.1f} minutes is sufficient for injection (min: {min_gap_minutes})"
        )
...
def uniform_injection_points(num_real_posts: int, num_injections: int) -> List[int]:
    """
    ...
    if num_real_posts == 0:
        logger.debug(
            "Called uniform_injection_points with num_real_posts=0, returning empty list"
        )
        return []
    ...
    return [
        ...
        if i * spacing < num_real_posts
    ]


def inject_into_timeline(
    real_posts: List[Dict], injectable_posts: List[Dict], strategy: Dict
) -> List[Dict]:
    """
    ...
    logger.debug(
        f"inject_into_timeline called with: {len(real_posts)} real posts, {len(injectable_posts)} injectable posts, strategy={strategy.get('type', 'unknown')}"
    )
    ...
    if not real_posts or (
        len(real_posts) == 1 and real_posts[0].get("stub_for_injection", False)
    ):
        logger.debug(
            "No real posts or only stub post but have injectable posts, returning only injectable posts"
        )
        ...
        if strategy.get("shuffle_injected", False):
            posts_to_inject = random.sample(
                injectable_posts, min(max_to_inject, len(injectable_posts))
            )
        else:
    ...
    if strategy_type == "uniform":
        injection_points = uniform_injection_points(
            len(sorted_real_posts), max_injections
        )
        ...
        injection_points = uniform_injection_points(max_first, max_injections)
    ...
    if strategy_type == "tag_match":
        ...
        for i, real_post in enumerate(sorted_real_posts):
            ...
            if matched_idx is not None and i < len(sorted_real_posts) - 1:
                ...
                if min_gap_minutes > 0 and not check_time_gap(
                    real_post, next_post, min_gap_minutes
                ):
                    continue
        ...
        for i, real_post in enumerate(sorted_real_posts):
            ...
            if (
                i in injection_points
                and injection_count < max_injections
                and injectable_idx < len(available_injections)
            ):
