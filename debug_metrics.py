...
def main():
    ...
    parser.add_argument('--host', default='http://localhost:5002', help='Corgi service host URL')
    parser.add_argument('--metrics-port', default=9100, type=int, help='Metrics server port')
    parser.add_argument('--iterations', default=2, type=int, help='Number of test iterations')
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print("  ℹ️ This might cause issues with recommendations that need DB access")
    except Exception as e:
    ...
    try:
        ...
        if metrics_response.status_code == 200:
            print(f"  ✅ Metrics endpoint is responding ({len(metrics_response.text)} bytes)")
        else:
            print(f"  ❌ Metrics endpoint failed: {metrics_response.status_code}")
    except Exception as e:
        print(f"  ❌ Metrics endpoint error: {e}")
        print("  ⚠️ Cannot reach metrics server - verify prometheus_client is configured correctly")
    ...
    try:
        ...
        timeline_url = f"{base_url}/api/v1/timelines/home?inject=false&user_id={user_id}"
            ...
            with requests.Session() as session:
                ...
                if response.status_code == 200:
                    try:
                        ...
                        injected_count = sum(1 for post in timeline if isinstance(post, dict) and post.get('injected'))
                        real_count = len(timeline) - injected_count
                        
                        print(f"  ✅ Got {len(timeline)} posts ({injected_count} injected, {real_count} real)")
                        
                        # Check if posts have the correct structure
                        if timeline and len(timeline) > 0:
                            sample_post = timeline[0]
                            print(f"  Post ID: {sample_post.get('id')}")
                            print(f"  Has 'injected' field: {'injected' in sample_post}")
                            print(f"  Has 'injection_metadata': {'injection_metadata' in sample_post}")
                            ...
                            if 'id' in sample_post:
                                ...
                                int_response = session.post(interaction_url, json=interaction_data, timeout=10, verify=args.verify_ssl)
                                
                                print(f"  Interaction response: {int_response.status_code}")
                                if int_response.status_code == 200:
                                    print(f"  ✅ Interaction logged successfully")
                                else:
                                    print(f"  ❌ Failed to log interaction: {int_response.status_code}")
    ...
    try:
        ...
        try:
            ...
            if response.status_code == 200:
                print("  ✅ Made request to trigger metrics collection via health endpoint")
            else:
                print(f"  ⚠️ Failed to trigger metrics collection: {response.status_code}")
    ...
    try:
        ...
        if metrics_response.status_code == 200:
            ...
            for metric in key_metrics:
                if metric in metrics_text:
                    ...
                    if count > 0:
                        ...
                        print(f"  ⚠️ {metric}: Found in metrics definition but no data points")
                        # Check if there's a definition line without data
                        definition_lines = [line for line in metrics_text.split('\n') 
            ...
            if not any(line.startswith('corgi_') for line in metrics_text.split('\n')):
                ...
                print("     - Check that the Prometheus client is correctly initialized in app.py")
                print("     - Verify metrics are being collected in the correct registry")
                print("     - Make sure the metrics server is running on the expected port")
        else:
