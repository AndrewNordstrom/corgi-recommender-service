...
class InteractionContext(BaseModel):
    ...
    source: Optional[str] = Field(None, description="Source of the interaction (e.g., timeline_home)")
    
class InteractionRequest(BaseModel):
    """Request model for logging a user interaction."""
    user_alias: Optional[str] = Field(None, description="Pseudonymized identifier for the user")
    user_id: Optional[str] = Field(None, description="Alternative direct user identifier")
    ...
    context: Optional[InteractionContext] = Field(None, description="Additional context about the interaction")
...
class MastodonPost(BaseModel):
    ...
    account: Account
    replies_count: Optional[int] = Field(0, description="Number of replies to this post")
    reblogs_count: Optional[int] = Field(0, description="Number of reblogs of this post")
    favourites_count: Optional[int] = Field(0, description="Number of favorites for this post")
    url: Optional[str] = Field(None, description="URL to the post")
    sensitive: Optional[bool] = Field(False, description="Whether the post contains sensitive content")
    spoiler_text: Optional[str] = Field("", description="Content warning text")
    visibility: Optional[str] = Field("public", description="Visibility level of the post")
    ranking_score: Optional[float] = Field(None, description="Recommendation ranking score (0.0 to 1.0)")
    recommendation_reason: Optional[str] = Field(None, description="Human-readable reason for recommendation")
    is_recommendation: Optional[bool] = Field(False, description="Whether this post was added as a recommendation")
    is_real_mastodon_post: Optional[bool] = Field(True, description="Whether this is a real Mastodon post (vs. synthetic)")
    is_synthetic: Optional[bool] = Field(False, description="Whether this is a synthetic post")

class TimelineResponse(BaseModel):
    """Response model for timeline endpoints."""
    timeline: List[MastodonPost]
    injected_count: Optional[int] = Field(None, description="Number of recommendations injected")

class PrivacySettings(BaseModel):
    """User privacy settings."""
    user_id: str = Field(..., description="User identifier")
    tracking_level: TrackingLevel = Field(..., description="Privacy level controlling data collection and usage")
...
@app.get("/api/v1/interactions/{post_id}", 
...
async def get_post_interactions(
    post_id: str = Path(..., description="The ID of the post to retrieve interactions for")
):
    """
...
async def get_home_timeline(
    user_id: Optional[str] = Query(None, description="The user ID to get timeline for (for testing/validation)"),
...
@app.get("/api/v1/timelines/home/augmented", 
...
async def get_augmented_timeline(
    user_id: Optional[str] = Query(None, description="The user ID to get timeline for (for testing/validation)"),
    limit: int = Query(20, description="Maximum number of posts to return"),
    inject_recommendations: bool = Query(False, description="Whether to inject personalized recommendations into the timeline")
):
    """
