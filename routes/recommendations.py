                ...
                with get_cursor(conn) as cur:
                    ...
                    for rec in recommendations:
                        cur.execute('''
                            ...
                            VALUES (?, ?, ?, ?)
                        ''', (rec["user_id"], rec["post_id"], rec["score"], rec["reason"]))
                    
                    conn.commit()
                    
                    logger.info(f"Generated {len(recommendations)} test rankings for user {user_alias}")
                    return jsonify({
    ...
    with get_db_connection() as conn:
        with get_cursor(conn) as cur:
            if USE_IN_MEMORY_DB:
                ...
                cur.execute("SELECT COUNT(*) FROM recommendations WHERE user_id = ?", (user_alias,))
                ...
                for row in ranking_data:
                    ...
                    try:
                        ...
                        if mastodon_post:
                            ...
                            post_data = {
                                "id": post_id,
                                "created_at": created_at.isoformat() if hasattr(created_at, 'isoformat') else created_at or datetime.now().isoformat(),
                                "account": {
                                    "id": author_id,
    ...
    if USE_IN_MEMORY_DB:
        ...
        with get_db_connection() as conn:
            with get_cursor(conn) as cur:
                ...
                cur.execute(f"SELECT COUNT(*) FROM post_metadata WHERE mastodon_post IS NOT NULL")
