...
def load_config():
    ...
    config_path = os.path.join(
        os.path.dirname(__file__), "..", "config", "cold_start_weights.json"
    )
...
def plot_tag_distribution(user_signals):
    ...
    temp_file = (
        f"/tmp/corgi_tag_distribution_{datetime.now().strftime('%Y%m%d%H%M%S')}.png"
    )
...
def plot_interaction_timeline(user_signals):
    ...
    if (
        not user_signals
        or "interaction_history" not in user_signals
        or not user_signals["interaction_history"]
    ):
    ...
    for action in action_types:
        ...
        plt.scatter(
            action_dates, y_values, label=action, color=action_colors[action], s=100
        )
    ...
    # Save to temporary file
    temp_file = (
        f"/tmp/corgi_interaction_timeline_{datetime.now().strftime('%Y%m%d%H%M%S')}.png"
    )
    plt.savefig(temp_file)
    plt.close()
...
def display_signal_profile(user_id, user_signals, verbose=False):
    ...
    if tags:
        ...
        print(
            tabulate(
                tag_table, headers=["Tag", "Count", "Percentage"], tablefmt="simple"
            )
        )
    else:
    ...
    if categories:
        ...
        for category, count in categories.most_common(5):
            category_table.append(
                [category, count, f"{count/interaction_count*100:.1f}%"]
            )
        print(
            tabulate(
                category_table,
    ...
    if "vibes" in user_signals and user_signals["vibes"]:
        ...
        print(
            tabulate(
                vibe_table, headers=["Vibe", "Count", "Percentage"], tablefmt="simple"
            )
        )
    ...
    if "tones" in user_signals and user_signals["tones"]:
        ...
        print(
            tabulate(
                tone_table, headers=["Tone", "Count", "Percentage"], tablefmt="simple"
            )
        )
    ...
    if verbose and "interaction_history" in user_signals:
        ...
        history_table = []
        for entry in sorted(
            user_signals["interaction_history"], key=lambda x: x.get("timestamp", 0)
        ):
...
def main():
    ...
    parser.add_argument(
        ...
        "-v",
        action="store_true",
        help="Show detailed information including full interaction history",
    )
    parser.add_argument(
        "--refresh", "-r", action="store_true", help="Refresh signals from database"
    )
    parser.add_argument(
        "--visualize", "-viz", action="store_true", help="Generate visualizations"
    )
