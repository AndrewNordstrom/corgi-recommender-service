...
from agents.user_profiles import get_profile_by_name, list_available_profiles, get_time_of_day
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description="Run a synthetic agent test against the Corgi Recommender.")
    ...
    parser.add_argument(
        "--output", 
        type=str, 
        help="Path to save the session output JSON"
    )
    ...
    parser.add_argument(
        "--max-tokens", 
        type=int,
        help="Maximum token usage limit (default: no limit)"
    )
...
def show_token_usage() -> None:
    ...
    if not TOKEN_TRACKING_AVAILABLE:
        logger.error("Token tracking module not available. Make sure token_tracker.py is properly installed.")
        sys.exit(1)
...
def run_agent_test(args) -> Dict[str, Any]:
    ...
    if args.list_profiles:
        ...
        for profile in profiles:
            ...
            print(f"    Browser tooling: {'Required' if profile['use_browser'] else 'Not required'}")
            print()
        sys.exit(0)
    ...
    try:
        if not args.profile:
            logger.error("Profile is required unless using --list-profiles or --show-usage flags")
            sys.exit(1)
        ...
        logger.info(f"Browser tooling: {'Required' if profile.use_browser else 'Not required'}")
    ...
    if args.privacy_level:
        if test_goal:
            ...
            test_goal = f"Test the Corgi Recommender with privacy_level={args.privacy_level}"
...
def print_agent_test_summary(results: Dict[str, Any]) -> None:
    ...
    print(f"Interactions Used: {results.get('interactions_used', 0)}/{results.get('max_interactions', '?')}")
    ...
    if 'token_usage' in results:
        ...
        if usage.get('token_limit'):
            ...
            print(f"Token Limit: {usage.get('total_tokens', 0)}/{usage.get('token_limit')} ({percent:.1f}%)")
    ...
    if 'logs' in results and 'actions' in results['logs']:
        ...
        if privacy_actions:
            for action in privacy_actions:
                ...
                if action['action_type'] == 'privacy_before_change':
                    print(f"  - Before change: {details.get('tracking_level', 'unknown')} ({timestamp})")
                elif action['action_type'] in ['privacy_change_simulated', 'heuristic_privacy_change']:
                    print(f"  - Changed from {details.get('old_level', 'unknown')} to {details.get('new_level', 'unknown')} ({timestamp})")
        else:
        ...
        if feedback_actions:
            for action in feedback_actions:
                if action['action_type'] == 'tool_response' or action['action_type'] == 'heuristic_feedback':
                    if action['action_type'] == 'tool_response':
                        response = action.get('details', {}).get('response', {})
                        if isinstance(response, dict) and 'observation' in response:
                            observation = response.get('observation', '')
                            if 'feedback' in observation.lower() or 'said:' in observation.lower():
                                print(f"  - {observation}")
                    else:
                        # Handle heuristic feedback
                        details = action.get('details', {})
                        if 'feedback' in details:
                            print(f"  - [Heuristic] Post {details.get('post_id', 'unknown')}: {details.get('feedback', '')}")
        else:
...
def analyze_logs(args) -> Dict[str, Any]:
    ...
    for profile, stats in runs_by_profile.items():
        if stats['total_runs'] > 0:
            stats['avg_success_rate'] = (stats['successful_runs'] / stats['total_runs']) * 100
            stats['avg_interactions'] = stats['total_interactions'] / stats['total_runs']
    ...
    results = {
        ...
        'avg_interactions_per_run': total_interactions / total_runs if total_runs > 0 else 0,
    ...
    if args.output:
        ...
        with open(report_path, 'w') as f:
            ...
            f.write(f"Average interactions per run: {results['avg_interactions_per_run']:.1f}\n")
            if total_tokens > 0:
                f.write(f"Total tokens used: {total_tokens}\n")
                f.write(f"Total estimated cost: ${total_cost:.6f}\n")
            f.write("\n=== PROFILE BREAKDOWN ===\n\n")
            
            for profile, stats in sorted(runs_by_profile.items(), key=lambda x: x[1]['total_runs'], reverse=True):
...
def print_log_analysis_summary(results: Dict[str, Any]) -> None:
    ...
    print("\n=== Profile Breakdown ===")
    for profile, stats in sorted(results.get('profiles', {}).items(), key=lambda x: x[1]['total_runs'], reverse=True):
