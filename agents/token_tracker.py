...
class TokenTracker:
    ...
    PRICING = {
        # Claude 3 models
        "claude-3-opus-20240229": {"input": 15.0, "output": 75.0},  # $15/$75 per million tokens
        "claude-3-sonnet-20240229": {"input": 3.0, "output": 15.0},  # $3/$15 per million tokens
        "claude-3-haiku-20240307": {"input": 0.25, "output": 1.25},  # $0.25/$1.25 per million tokens
        ...
        "claude-instant-1.2": {"input": 1.63, "output": 5.51},  # $1.63/$5.51 per million tokens
        
        # Default fallback (use Opus pricing as a conservative estimate)
        "default": {"input": 15.0, "output": 75.0}  # $15/$75 per million tokens
    }
    
    def __init__(self, max_tokens: Optional[int] = None, log_file: Optional[str] = None):
        ...
        # Set up logging
        self.log_file = log_file or os.path.join(os.path.dirname(os.path.dirname(__file__)), 
    ...
    def record_usage(self, model: str, input_tokens: int, output_tokens: int, 
                     request_duration: Optional[float] = None) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            # Log the usage
            self.logger.info(f"API request: {model}, in={input_tokens}, out={output_tokens}, "
                          f"cost=${cost:.6f}, total=${self.total_cost:.6f}")
            
            # Check if we've reached the token limit
            total_tokens = self.total_input_tokens + self.total_output_tokens
            if self.max_tokens and total_tokens >= self.max_tokens and not self.token_limit_reached:
                self.token_limit_reached = True
                self.logger.warning(f"⚠️ TOKEN LIMIT REACHED: {total_tokens}/{self.max_tokens}")
    ...
    def log_summary(self) -> None:
        ...
        if summary['token_limit']:
            ...
            self.logger.info(f"Token limit: {summary['total_tokens']}/{summary['token_limit']} ({percent:.1f}%)")
        
        self.logger.info("===== MODEL BREAKDOWN =====")
        for model, stats in summary['usage_by_model'].items():
            self.logger.info(f"{model}: {stats['requests']} requests, "
    ...
    def save_usage_to_file(self, file_path: Optional[str] = None) -> str:
        ...
        if file_path is None:
            ...
            file_path = os.path.join(os.path.dirname(self.log_file), f"token_usage_{timestamp}.json")
