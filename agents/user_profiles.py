...
class UserProfile:
    ...
    def get_behavior_prompt(
        self, time_of_day: Optional[str] = None, test_goal: Optional[str] = None
    ) -> str:
        ...
        if action_type == "rating":
            ...
            return {
                "decision": (
                    "like"
                    if any(
                        topic in post_content.lower() for topic in self.preferred_topics
                    )
                    else "skip"
                )
            }
            ...
            return {
                "decision": random.choice(
                    ["scroll", "favorite", "bookmark", "view_details"]
                )
            }
...
class TechFan(UserProfile):
    ...
    def rate_recommendation(self, post_content: str) -> str:
        ...
        matching_topics = [
            topic for topic in self.preferred_topics if topic in post_lower
        ]
...
class NewsSkeptic(UserProfile):
    ...
    def rate_recommendation(self, post_content: str) -> str:
        ...
        matching_skeptical = [
            topic for topic in self.skeptical_topics if topic in post_lower
        ]
    ...
    def make_heuristic_decision(
        ...
        if action_type == "rating":
            ...
            matching_preferred = [
                topic for topic in self.preferred_topics if topic in post_content
            ]
            matching_skeptical = [
                topic for topic in self.skeptical_topics if topic in post_content
            ]
            ...
            if matching_preferred and not matching_skeptical:
                ...
                return {
                    "decision": "skip",
                    "reason": f"contains {matching_skeptical[0]}",
                }
            else:
                # Neutral content gets a mixed response
                return {
                    "decision": random.choice(["view", "skip"]),
            ...
            return {
                "decision": random.choice(["limited", "none"]),
            ...
            return {
                "decision": random.choice(
                    ["scroll", "scroll", "scroll", "view_details"]
                )
            }
    ...
    def make_heuristic_decision(
        ...
        if action_type == "rating":
            ...
            if "corgi" in post_content:
                return {
                    "decision": random.choice(["favorite", "bookmark"]),
                    "reason": "corgi content!",
                }
...
class PrivacyTester(UserProfile):
    ...
    def get_behavior_prompt(
        ...
        """
        privacy_level = (
            test_goal.split("privacy_level=")[1]
            if test_goal and "privacy_level=" in test_goal
            else None
        )
    ...
    def handle_post_batch(self, post_contents: List[str]) -> Dict[str, str]:
        ...
        for post in post_contents:
            if privacy_level == "full":
                ...
                results[post] = (
                    "Interesting recommendation. I'm evaluating how it relates to my privacy settings."
                )
    ...
    def make_heuristic_decision(
        ...
        if action_type == "rating":
            ...
            return {
                "decision": random.choice(["view", "favorite"]),
                "reason": "testing recommendation behavior",
            }
            ...
            if current_level == "full":
                ...
                return {
                    "decision": "full",
                    "reason": "resetting to full tracking to compare",
                }
