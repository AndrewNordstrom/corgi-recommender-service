...
class ClaudeInterface:
    ...
    def __init__(self, token_tracker=None):
        ...
        if not self._validate_api_connection():
            raise ConnectionError("Could not connect to Claude API. Please check your API key and internet connection.")
    ...
    def _validate_api_connection(self) -> bool:
        ...
        try:
            ...
            if "Ready" in response.get("content", []):
                ...
                self.logger.warning("Claude API connection validation failed: unexpected response")
                return False
    ...
    def process_with_tools(self, 
        ...
        # Log the request (excluding API key)
        safe_payload = payload.copy()
        self.logger.debug(f"Sending tool-enabled request to Claude API: {json.dumps(safe_payload)}")
