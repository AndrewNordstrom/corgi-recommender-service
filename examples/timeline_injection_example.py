        ...
        injectable_posts.append(
            {
                ...
                + "Z",  # Old timestamp that will be replaced
...
def main():
    ...
    timeline_uniform = inject_into_timeline(
        real_posts, injectable_posts, strategy_uniform
    )
    print_timeline(timeline_uniform, "Uniform Strategy Timeline")

    # Strategy 2: After N posts
    strategy_after_n = {"type": "after_n", "n": 2, "max_injections": 4}

    timeline_after_n = inject_into_timeline(
        real_posts, injectable_posts, strategy_after_n
    )
    print_timeline(timeline_after_n, "After N Strategy Timeline")

    # Strategy 3: First only
    strategy_first_only = {"type": "first_only", "max_injections": 2}

    timeline_first_only = inject_into_timeline(
        real_posts, injectable_posts, strategy_first_only
    )
    print_timeline(timeline_first_only, "First Only Strategy Timeline")

    # Strategy 4: Tag matching
    strategy_tag_match = {"type": "tag_match", "max_injections": 3}

    timeline_tag_match = inject_into_timeline(
        real_posts, injectable_posts, strategy_tag_match
    )
    ...
    timeline_with_gap = inject_into_timeline(
        real_posts, injectable_posts, strategy_with_gap
    )
