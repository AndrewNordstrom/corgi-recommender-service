...
TOPIC_TAGS = [
    "technology", "tech", "programming", "coding", "developer",
    "art", "photography", "design", "illustration", "creative",
    "science", "space", "research", "nature", "climate",
    "books", "reading", "writing", "literature", "publishing",
    "music", "movies", "film", "tv", "entertainment",
    "gaming", "games", "videogames", "boardgames", "rpg",
    "food", "cooking", "baking", "recipe", "cuisine",
    "health", "fitness", "wellness", "mentalhealth", "running",
    "history", "philosophy", "politics", "economics", "culture",
    "travel", "adventure", "outdoors", "hiking", "backpacking"
]
...
def convert_post_to_cold_start_format(post, source_instance):
    ...
    account = {
        ...
        "display_name": post['account'].get('display_name', post['account'].get('username', 'Unknown User')),
        "url": post['account'].get('url', f"https://{source_instance}/@{post['account'].get('username', 'unknown')}"),
        "avatar": post['account'].get('avatar', 'https://mastodon.social/avatars/original/missing.png'),
        "avatar_static": post['account'].get('avatar_static', post['account'].get('avatar', 'https://mastodon.social/avatars/original/missing.png')),
        "header": post['account'].get('header', 'https://mastodon.social/headers/original/missing.png'),
        "header_static": post['account'].get('header_static', post['account'].get('header', 'https://mastodon.social/headers/original/missing.png')),
    ...
    for tag in tags:
        tag_lower = tag.lower()
        if any(topic in tag_lower for topic in ["tech", "code", "program", "develop", "software"]):
            category = "technology"
            break
        elif any(topic in tag_lower for topic in ["art", "photo", "design", "creative", "illustration"]):
            category = "art"
            break
        elif any(topic in tag_lower for topic in ["science", "space", "research", "nature"]):
            category = "science"
            break
        elif any(topic in tag_lower for topic in ["book", "read", "write", "literature"]):
            category = "books"
            break
        elif any(topic in tag_lower for topic in ["music", "movie", "film", "tv", "entertainment"]):
            ...
            category = "food"
            break
        elif any(topic in tag_lower for topic in ["health", "fitness", "wellness", "mental"]):
            category = "health"
            break
...
def fetch_diverse_posts(count=100):
    ...
    final_posts.sort(key=lambda x: (x.get('favourites_count', 0) + x.get('reblogs_count', 0) * 2), reverse=True)
